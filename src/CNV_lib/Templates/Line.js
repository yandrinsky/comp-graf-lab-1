...
class Line extends Standard{
    constructor(config) {
        ...
        if(config.equation){
            const {start, end, check} = this.getCanvasIntersectionCoordsFromEquation({isShift: true});
            this.start = start;
    ...
    getCanvasIntersectionCoordsFromEquation({isShift = false}) {
        ...
        equation = equation.map((el ,index) => {
            if(index === 0) {
                if(el === "+"){
                    return 1;
                } else if(el === "-") {
        ...
        if(isShift) {
            equation[1] += (-1) * getSign(equation[0]) * Store.state.shift.x * Number(!!equation[0])
                + Store.state.shift.y;
        }
        ...
        } else if(bottomSideIntersection > 0 && bottomSideIntersection < Store.state.canvas.width) {
    ...
    getEquation() {
        const {start: {x: x1, y: y1}, end: {x: x2, y: y2}} = this.getCoords();
        console.log({...this.getCoords()})
        return getEquationFrom2Points(x1, y1, x2, y2);
    }

    getShiftEquation() {
        const {start: {x: x1, y: y1}, end: {x: x2, y: y2}} = this.getShiftCoords();
        return getEquationFrom2Points(x1, y1, x2, y2);
    }
